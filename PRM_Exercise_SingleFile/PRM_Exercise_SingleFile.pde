...
/*
TODO:
  done 1. Add a function so that when you press the 'r' key, the obstacles move to
  ...
  done 2. Currently edge connections of any length are allowed. Update the connectNeighbors()
     function to only allow edges if their length is less than 200 pixels. What use is there
     for this? Why do you think people sometimes put a maximum length on PRM edges? to find a better optimized path in terms of distance
  done 3. The function, closestNode() is supposed to return the ID of the PRM node 
  ...
  done 4. You can see we define a box obstacle with a top left corner, and a width and height.
     I have already written code to draw this box with the rect() command. Uncomment
     the code, to see the box drawn.
  done 5. For the PRM to be collision free wrt the box we need to make sure no nodes are
  ...
  done 6. Pressing the arrow keys will move the pink box. Update the code to make
...
int closestNode(Vec2 point){
  ...
  int closestPoint = 0;
  float currentMinDist = point.distanceTo(nodePos[0]);
  for(int i=0; i < numNodes; i++) {
...
boolean pointInBox(Vec2 boxTopLeft, float boxW, float boxH, Vec2 pointPos){
  ...
  boolean isXInside = false, isYInside = false;
...
void connectNeighbors(){
  for (int i = 0; i < numNodes; i++){
    ...
    for (int j = 0; j < numNodes; j++){
      ...
      if(distBetween > 200) { // if connection length is greather than 200 pixels, don't connect 
        continue;
      }
