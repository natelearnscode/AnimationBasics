...
/*
TODO:
  done (double check) 1. Currently, there are no forces acting on the agents. Add a goal force that penalizes
  ...
  done 1a. Scale the goal force by the value k_goal. Find a value of k_goal that lets the agents
     smoothly reach their goal without overshooting it first.
     i.e., goal_force = k_goal*(goal_vel-current_vel); acc += goal_force
  done 2. Finish the function computeTTC() so that it computes the time-to-collision (TTC) between
  ...
  done 2a. Test computeTTC() with some scenarios where you know the (approximate) true value.
  done 3. Compute an avoidance force as follows:
   ...
   done 4. Make the following simulation improvements:
   ...
   done 5. Finally, place 2 more agents in the scene and try to make an interesting scenario where the
...
void setup(){
  ...
  agentPos[3] = new Vec2(400,350);
  agentPos[4] = new Vec2(200,400);

  goalPos[0] = new Vec2(200,420);
  goalPos[1] = new Vec2(120,120);
  goalPos[2] = new Vec2(220,220);
  goalPos[3] = new Vec2(600,600);
  goalPos[4] = new Vec2(500,0);
...
float computeTTC(Vec2 pos1, Vec2 vel1, float radius1, Vec2 pos2, Vec2 vel2, float radius2){
  ...
  float time = rayCircleIntersectTime(pos2, combinedRadius, pos1, dir);
  return time;
}
...
Vec2 computeAgentForces(int id){
  ...
  // if agent has reached the goal (within a certain threshold) no other forces are applied
  if(goalVel.length() < 1) {
    return acc;
  }
  ...
  for(int i=0; i< numAgents; i++) {
    //disregard when i is the id
    if(i != id) {
      // compute ttc for id and neighbors
      ...
      Vec2 B_future = agentPos[i].plus(agentVel[i].times(ttc));

      // calculate relative velocity at moment of collision
      Vec2 relative_future_direction = (A_future.minus( B_future)).normalized();
      
      // calculate avoidance force
      acc = acc.plus(relative_future_direction.times(k_avoid * (1/ttc)));
...
void draw(){
  ...
  fill(20,200,150);
  for (int i = 0; i < numAgents; i++){
    circle(agentPos[i].x, agentPos[i].y, agentVisualRad*2);
  }
}
