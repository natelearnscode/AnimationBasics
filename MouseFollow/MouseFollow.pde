...
//  done 1. Initially, the code shows a dark red circle with a purple-ish outline.
//     Change the program to draw a red circle with a black outline instead. 
//  done 2. Find the Processing.org reference pages for the circle command. 
//     Why am I multiplying the radius by 2? (Note: these reference pages are 
//     very useful, be sure to check them out for general Processing help.) the third argument is the width and height of the circle so we want the diameter aka 2*radius
//  done 3. Change the background color to be white instead green.
//  done 4. Have the ball follow the user's mouse. You should do this by setting
//      dir to be the vector pointing from the ball's current position (pos)
//      to the mouse's current position (mousePos).
//  done 5. Try to get some keyboard input working:  
...
//  done 6. The simple approach is Step 4 will leave the ball jittering when it
//     reaches the mouse. Find a strategy to fix the jitter so the ball stops
//     completely when it is nicely centered on the mouse.
//  done 7. Currently, the variable 'speed' doesn't have a well defined unit and its
...
float speed = 100;
float snapToMouseThreshold = 10; //if within this amount of pixels than snap to mouse to avoid jitter

void update(float dt){
  Vec2 mousePos = new Vec2(mouseX, mouseY);
  Vec2 dir = mousePos.minus(pos); //Should be vector pointing from pos to MousePos
  if (dir.length() > snapToMouseThreshold){ //if speed*dt > dir.length()
    dir.normalize();
    vel = dir.times(speed * dt); // update speed based on time that has passed
    pos.add(vel);
  }
  else {
   vel = new Vec2(0, 0);
