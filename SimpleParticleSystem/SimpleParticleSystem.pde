...
//TODO:
//  done 1. The balls start red, make them blue instead.
//  done 2. Randomize the initial particle velocities so that xVel starts in the range (30,90),
//     and yVel starts in the range (-190, -200)
//  done 3. There is currently a small cap on the number of particles that can be spawned. 
//     Raise it up to 400.
//  done 4. Currently, pressing the 'r' key prints that it's resetting the particle system,
//     but it doesn’t actually reset anything yet. Fix that by having the simulation
//     reset when the user presses 'r'.
//  done 5. Pressing the arrow keys will move the big red ball, but sometimes we would like
...
//  done 6. The red ball only moves up/down/left/right. Change the code so that is two keys
//     are pressed simultaneously the red ball will move diagonally.
//  done 7. A common pitfall in games is that the diagonal motion is actually faster than
...
//  done 8. The small blue balls (particles) have momentum, but they are missing the effect
...
//  done 9. The current code for bouncing the particles off of the red ball assumes the red
...
//  done 2. Change the color of the particles over time
...
void update(float dt){
  ...
  for (int i = 0; i < toGen; i++){
    ...
    vel[numParticles] = new Vec2(random(30, 91),random(-200, -189));
    life[numParticles] = 0;
    numParticles += 1;
  }
  ...
  obstacleVel = (obstacleVel.normalized()).times(obstacleSpeed);
  spherePos.add(obstacleVel.times(dt));
  
  for (int i = 0; i <  numParticles; i++){
    
    //update life time value
    life[i] += dt; 
...
void keyReleased(){
  if (key == 'r'){
    ...
    //reset particles, in a language like C++ I would manually delete array, but here I let the garbage collector do it
...
void draw(){
  ...
  stroke(0,0,0);
  for (int i = 0; i < numParticles; i++){
    //start off as light blue and more saturated blue over lifespan
    fill(200 - (life[i]/maxLifeTime)*200, 200 - (life[i]/maxLifeTime)*200, 256);
...
public class Vec2 {
  ...
  public void normalize(){
    float magnitude = sqrt(x*x + y*y);
    if(magnitude != 0){ // acounting for divide by zero error
      x /= magnitude;
      y /= magnitude;
    }
  }
  ...
  public Vec2 normalized(){
    float magnitude = sqrt(x*x + y*y);
    if(magnitude == 0) { // accounting for divide by zero error
      return new Vec2(x, y);
    }
